\message{ !name(lawvere.tex)}% Enriched Lawvere Theories for Operational Semantics
...
In an theory $\T$ enriched in a category $\V$ of some kind of ``directed object'', including graphs, categories, and posets, the theory has the following interpretation:
...
Because operational semantics uses (labelled) directed graphs to represent terms and rewrites, this should be our main example of enriching category; i.e. the ``thing'' of $n$-ary operations, or $n$-variable terms in a theory, should be a directed graph whose edges are rewrites. However, for reasons to be explained, we need to use a slightly different concept called a \emph{simplicial set}. For all intents and purposes, it can be understood as an ordinary directed graph; although, it does provide a more general and powerful framework for higher-dimensional rewriting, which should prove to be quite useful.

We denote by $\sSet := [\Delta^\op, \Set]$ to be the category of simplicial sets, contravariant functors from the category of linear orders and monotone maps to the category of sets and functions, and natural transformations between them. They can be visualized as spaces built from ``simplices'', which are triangles of any dimension. We will only need those simplicial sets which have ``nondegenerate'', or nontrivial, elements in dimension 1. These can be called  For a gentle introduction to simplicial sets, see \cite{sset}.
...
\begin{itemize}
\item 
\textbf{Simplicial Sets}: $\sSet$-theories represent ``small-step" operational semantics: \\ --- a hom-edge is a \textit{single} term rewrite.
\item
...
\end{document}
\message{ !name(lawvere.tex) !offset(-1264) }
