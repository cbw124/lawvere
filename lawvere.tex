...
Power introduced the notion of enriched Lawvere theory about twenty years ago, ``in seeking a general account of what have been called notions of computation'' \cite{power}. The original definition is as follows: for a symmetric monoidal closed category $(\V,\otimes,I)$, a ``$\V$-enriched Lawvere theory'' is a $\V$-category $\T$ that has powers by objects in $\V_f$, equipped with an identity-on-objects $\V$-functor 
\[  \tau\maps\V_f^\op \to \T \]
...
In the same way that all the objects of a Lawvere theory are $\NN$-powers of a generating object, the objects of a $\J$-$\V$ theory are $\J$-powers $t^J$ of a generating object $t = \tau(I)$, one for each $J \in \J$.  (BUT IN THE LAST SECTION WE SAID WE'D BE USING $1$ AS OUR NAME FOR THE MONOIDAL UNIT!)  Note that $t$ itself is $t^I$.  Just as every $n\in \NN^\op$ is a power of $1$, every $J\in \tilde{\J}^{\op}$ is a power of the monoidal unit $I\in \V$, i.e. using Eq. (\ref{eq:co-power}) for $\tilde{\J}^{\op}$, $(\star \mapsto 1_J) \in [I,\tilde{\J}^{\op}(J,J)]$ is sent to the canonical isomorphism:
\begin{equation}
J \cong I^J.
\end{equation}
...
The usual kinds of arities are eleutheric: in particular, finite cardinals.  We take these as the objects of our system of arities $\J$.  WHEN???  STARTING RIGHT NOW???

Let $(\V,\times,I_\V)$ be a cartesian closed category with finite coproducts of $I_\V$. Define $\NN_\V$ to be the full subcategory of finite coproducts of the unit object:
\[   n_\V := n \cdot I_\V = \sum_{i \in n} I_\V  \]
...
\begin{example}
	...
	\textbf{rewrites} & \alpha_{MMM}\colon m \circ (m \times \id_M) \Rightarrow m \circ (\id_M \times m) & \text{associator}\\
	...
	\textbf{coherence laws} & (M\times\alpha)\circ \alpha_{MmM}\circ (\alpha\times M) = \alpha_{MMm}\circ \alpha_{mMM} & \text{pentagon identity}\\
	& (M\times \lambda) \circ \alpha_{M1M} = \rho\times M & \text{triangle identity}\\
	\end{array}\]
\end{example}
NOT OBVIOUS FROM HERE WHERE $\alpha_{MmM}$ and $\alpha_{M1M}$ come from!!!
