...
In this paper, however, we only consider \textit{natural number} arities, while still retaining enrichment. To do this we use the work of Lucyshyn-Wright \cite{rbb}, who has generalized
...
\end{definition}
\noindent
This definition is adapted from that of Lucyshyn-Wright \cite{rbb}.  While he demands that $\tau$ be the identity on objects, we allow it to be bijective on objects, to handle change-of-base.

\begin{definition}
A \textbf{model} of $\T$ in a $\V$-category $\C$ is a $\NN_\V$-preserving $\V$-functor 
\[  \mu \maps \T \to \C.\]
\end{definition}

In the same way that all the objects of a Lawvere theory are $\NN$-powers of a generating object, all the objects of a $\V$-theory are $\NN_\V$-powers $t^{n_\V}$ of a generating object $t = \tau(1_\V)$, one for each $n_\V \in \NN_\V$.  Note that $t$ itself is $t^{1_\V}$.  Moreover, just as every $n\in \NN^\op$ is a power of $1$, every $J\in \NN_\V^\op$ is a power of $1_\V$.  To see this, note that using Eq.\ (\ref{eq:co-power}) for $\NN_\V^\op$, we obtain a natural isomorphism
\begin{equation}
n_\V \cong 1^{n_\V}.
\end{equation}
This is just the opposite of the usual isomorphism $n_\V \cong n_\V^1$ in $\NN_\V$.   Then, since $\tau$ preserves $\NN_\V$-powers and is bijective on objects, this implies that every object of $\T$ is an $\NN_\V$-power of $t = \tau(1_\V)$.

There is a category $\V\Law$ whose objects are $\V$-theories and whose morphisms are $\NN_\V$ power-preserving $\V$-functors $f\maps \T \to \T'$ such that $f\tau = \tau'$. For every $\V$-theory $\T$ and every $\V$-category $\C$ with $\J$-powers, we define the category of models $\Mod(\T,\C)$ to consist of $\NN_\V$-power preserving $\V$-functors $\T\to \C$ and $\V$-natural transformations. If  $\V$ is a \textbf{cosmos}, i.e.\ complete and cocomplete, then the functor categories of $\V\Cat$ are also $\V$-categories, including $\V\Law$ and $\Mod(\T,\C)$. This is potentially very useful, and the enriching categories $\V = \Gph, \Cat, \Pos, \Set$ that are the focus of this paper are indeed cosmoi.
...
How exactly does the ``free-forgetful'' $\V$-adjunction work, going between $\V$ and 
models of $\T$ in $\V$?
\[\begin{tikzcd}[column sep=small]
...
\end{tikzcd}\]
Lucyshyn-Wright \cite[Sec.\ 8]{rbb} has described the \textbf{free model} of a $\V$-theory.  This is the enriched generalization of the free model described in $\S$2: it is the composite of $\tau^\op\maps \NN_\V \to \T^\op$ with the Yoneda embedding $y\maps \T^\op \to [\T,\V]$ sending each $n_\V \in \NN_\V$ to its representable $\V$-functor:
\[\begin{array}{rllll}
...
Since an object of $\V$ does not necessarily have a ``poset of finite subobjects'' over which to take a filtered colimit (as in $\Set$), the extension of the free model to all of $\V$ is specified by a somewhat higher-powered generalization: the ``free model'' functor on $\V$ is the left Kan extension of $y\tau$ along $j$:
\[\begin{tikzcd}
...
\end{tikzcd}\]
This is essentially the universal ``best solution'' to the problem of making the triangle commute up to a natural transformation: i.e.\ for any other functor $G \maps \V \to [\T,\V]$ and $\V$-natural transformation $\theta \maps y\tau \Rightarrow Gj$, the latter factors uniquely through the unit $\eta$.
