...
% 2019/5/15 - changes after first arXiv version
...
Because operational semantics uses (labelled) directed graphs to represent terms and rewrites, this should be our main example of enriching category; i.e. the ``thing'' of $n$-ary operations, or $n$-variable terms in a theory, should be a directed graph whose edges are rewrites. However, for reasons to be explained, we need to use a slightly different concept called a \emph{simplicial set}. For our purposes, it can be understood as an ordinary directed graph -- though it does provide a more general framework for higher-dimensional rewriting, which may be quite useful.

We denote by $\sSet := [\Delta^\op, \Set]$ the category of simplicial sets, contravariant functors from the category of finite linear orders and monotone maps to the category of sets and functions, and natural transformations between them. They can be visualized as spaces built from ``simplices'', which are triangles of any dimension. For an introduction to simplicial sets, see \cite{sset}. We will only need those simplicial sets which have ``nondegenerate'', or nontrivial, elements in dimension 1 -- and finite products thereof. These we give the simpler name \textbf{unital graphs}, and we denote the full subcategory of unital graphs by $\uGph$. Hereafter, by ``graph'' we mean unital graph, though it will usually be explicit.

% % A key example for us is $\Gph$, the category of reflexive graphs.  This is $\Set^\R$, where $\R$ is the category with two objects $v$ and $e$, two morphisms $s,t \maps e \to v$, and a morphism $i \maps v \to e$ obeying $si = ti = 1_v$.   A reflexive graph has directed edges and allows multiple edges between any pair of vertices; further, every vertex is equipped with a distinguished self-loop.    A category enriched over $\Gph$, or $\Gph$-category, has a set of objects and for any two objects a reflexive graph called a ``hom-graph''.  The vertices of this hom-graph represent morphisms, but there are also edges between these morphisms.  $\Gph$-categories have a clear connection to the original idea of operational semantics:

\vskip 0.5em
In general, for a cartesian closed category $\V$, we take a ``$\V$-theory'' to be a $\V$-enriched Lawvere theory with natural number arities. We consider $\V$ as a choice of ``method of computation'' for algebraic theories. There are functors between enriching categories that allow the translation between different kinds of operational and denotational semantics.  There is a spectrum of choices for $\V$, which allow us to examine the computation of term calculi at various levels of detail:

\begin{itemize}
\item 
\textbf{Unital Graphs}: $\uGph$-theories represent ``small-step" operational semantics: \\ --- a hom-edge is a \textit{single} term rewrite.
\item
...
In Section \ref{sec:base_change} we discuss how suitable functors between enriching categories induce \textit{change-of-computation}: they transform theories, and their models, from one method of rewriting to another.   Our main examples arise from this chain of adjunctions:
\[\begin{tikzcd}[column sep=small]
\uGph \arrow[bend right,swap]{rr}{R}
& \ld &
\arrow[bend right,swap]{ll}{N} \Cat \arrow[bend left,below]{rr}{\FP}
& \ld &
\arrow[bend left,above]{ll}{\UC} \Pos \arrow[bend left,below]{rr}{\FS}
& \ld &
\Set \arrow[bend left,above]{ll}{\UP}
\end{tikzcd}\]
The right adjoints here automatically preserve finite products, and the left adjoints do as well:
\begin{itemize}
\item 
$R \maps \uGph \to \Cat$, called \textit{realization}, maps a unital graph $G$ to the category whose objects are vertices of $G$, whose morphisms are sequences of composable edges in $G$, and composition is concatenation, modulo equations for associativity and unitality. (This is where the distinction between graphs and unital graphs matters.)  Change of base along $R$ maps small-step operational semantics to big-step operational semantics.
\item
$\FP \maps \Cat \to \Pos$ maps a category $C$ to the poset whose elements are objects of $C$, with $c \le c'$ iff $C$ has a morphism from $c$ to $c'$.
Change of base along $\FP$ maps big-step operational semantics to full-step operational semantics.
\item
$\FS \maps \Pos \to \Set$ maps a poset $P$ to the set of ``components'' of $P$, where $p,p' \in P$ are in the same component if $p \le p'$.
...
There are just a few more technicalities. A category is \textbf{locally finitely presentable} if it is the category of models for a finite limits theory, and an object is \define{finite} if its representable functor is \define{finitary}: that is, it preserves filtered colimits \cite{adamekrosicky}.   A $\V$-category $\C$ is \textbf{locally finitely presentable} if its underlying category $\C_0$ is locally finitely presentable, $\C$ has finite powers, and $(-)^x\maps \C_0 \to \C_0$ is finitary for all finitely presentable $x$.  The details are not crucial here: all categories to be considered are locally finitely presentable. We will use  $\V_f$ to denote the full subcategory of $\V$ of finite objects: in $\uGph$, these are simply unital graphs with finitely many vertices and edges.
...
The basic monadicity results for Lawvere theories generalize to $\V$-theories when $\V$ is  complete and cocomplete, as in the main examples we consider: $\V = \uGph, \Cat, \Pos,$ and $\Set$.   Under this extra assumption $\V\Law$ and  $\Mod(\T,\C)$ can be promoted to 
...
in $\uGph,\Cat,\Pos$ and of course $\Set$, but not in $\V = \Set^k$, where
...
Hence, any cartesian functor that preserves chosen finite coproducts of the terminal object gives a ``change of computation''.  That is, it provides for a method of translating formal languages between various ``modes of operation''.   Moreover, this reasoning generalizes to \textbf{multisorted} $\V$-theories, enriched theories which have multiple sorts: given any $n\in \mathbb{N}$, the monoidal subcategory $(\NN_\V)^n$ is also an eleutheric system of arities, so Lucyshyn-Wright's monadicity theorem still applies.   In Section \ref{ssec:bisimulation} we show how this is useful in the study of bisimulation.
...
A model of this theory is a power-preserving $\uGph$-functor $\mu\maps \Th(\mathsf{SKI}) \to \uGph$. This gives a (unital) graph $\mu(t)$ of all terms and rewrites in the $SKI$-calculus as follows:
\[\begin{tikzcd}
...
This is how $\mu$ realizes $\Th(\mathsf{SKI})$ as a (unital) graph of terms and rewrites: in the same way that a natural transformation of two constant functors $a\Rightarrow b\maps 1\to \C$ is a morphism $a(1)\to b(1)$ in $\C$, a rewrite of closed terms $a\Rightarrow b\maps 1\to \mu(t)$ corresponds to an edge in $\mu(t)$:
\[\begin{tikzcd}
...
This process is intuitive, but how do we actually define the model, as a functor, to pick out a specific (unital) graph? There are many models of $\Th(\mathsf{SKI})$, but in particular we care about the canonical \textit{free} model, which means that $\mu(t)$ is simply the graph of all closed terms and rewrites in the $SKI$-calculus. This utilizes the enriched adjunction of Thm.\ \ref{thm:monadicity}:
\[\begin{tikzcd}
\phantom{AAAA} \underline{\uGph} \arrow[bend left=20,"f_\uGph"]{rr}
& {\phantom{Aa} \ld} &
\arrow[bend left=20,"u_\uGph"]{ll} \underline{\Mod}(\Th(\mathsf{SKI}),\uGph)
\end{tikzcd}\]

Then the canonical model of closed terms and rewrites is simply the free model on the empty unital graph, $f_\uGph(\emptyset)$, i.e. the $\V$-functor $\T(1,-)\maps\T\to \V$. Hence for us, the syntax and semantics of the $SKI$ combinator calculus are unified in the model $$\mu_{SKI}^\uGph:= \Th(\mathsf{SKI})(1,-)\maps \Th(\mathsf{SKI}) \to \uGph.$$ Here we reap the benefits of the abstract construction: the graph $\mu_{SKI}^\uGph(t)$ is the \textit{transition system} which represents the \textbf{small-step operational semantics} of the $SKI$-calculus: $$(\mu(a) \to \mu(b) \in \mu_{SKI}^\uGph(t)) \iff (a \Rightarrow b \in \Th(\mathsf{SKI})(1,t)).$$
...
We can now characterize the translation from small-step to \textbf{big-step} operational semantics. First, we describe the functor from graphs to categories which intuitively expresses this translation; but there is a problem with this functor - we then explain the solution in terms of unital graphs.

% Letting $i\maps \Delta\to \Cat$ denote the inclusion of linear orders into categories, the ``realization'' functor $R\maps \uGph\to \Cat$ is computed as a quotient of a sum:
...
The ``free category'' functor $\FC\maps \Gph \to \Cat$ gives for every graph $G$ the category $\FC(G)$: %whose objects are the vertices of $G$, and whose morphisms are freely generated by the edges of $G$, i.e. sequences 
\[\begin{array}{rl}
...
However, this functor does \emph{not} preserve products. Let $G_1 = {e\maps 0\to 1}$, the graph with one (nonreflexive) edge. Consider the product $G_1\times G_1$:

\[\begin{tikzcd}
    (0,0) \arrow[r,"(0{,}e)"] \ar[d,"(e{,}0)",swap] \ar[dr,"(e{,}e)"] & (0,1) \ar[d,"(e{,}1)"]\\
    (1,0) \ar[r,"(1{,}e)",swap] & (1,1)\\
  \end{tikzcd}\]

The labels 0 and 1 in the edge components refer to the reflexive edges in $G_1$. Essentially the problem is that these two triangles ``should commute'' - in the resulting category, the outer composites should equal the diagonal. However, since we are forming the \emph{free} category, no such equation is imposed.

In the unital graph $G_1\times G_1$, however, the fact that these triangles ``commute'' is represented by two pairs of degenerate 2-simplices in the product simplicial set. As a result, the realization functor $R\maps \uGph\to \Cat$ \emph{does} preserve products. The realized category $R(G_1\times G_1) \simeq R(G_1)\times R(G_1)$ looks exactly like the free category on $G_1\times G_1$ as a graph, except the unitality of identities in the product is respected.
...
Moreover, these equipments can be added or removed as needed: using change-of-theory, we can utilize a ``free reduction'' $\uGph$-functor $f_R\maps\Th(\mathsf{SKI})\to \Th(\mathsf{SKI}+\mathsf{R})$:
\[\begin{array}{rrcl}
...
\end{array}\]
This essentially interprets ordinary $SKI$ as having every subterm be a reduction context. This is a $\uGph$-functor because its hom component consists of graph-homomorphisms $$f_{n,m}\maps \Th(\mathsf{SKI})(t^n,t^m) \to \Th(\mathsf{SKI}+\mathsf{R})(t^n,t^m)$$ which simply send each application to its postcomposition with $R$, and each rewrite to its ``marked'' correspondent.
...
\bibitem{sset} G. Friedman, An elementary introduction to simplicial sets. Available as \href{https://arxiv.org/abs/0809.4221}.
