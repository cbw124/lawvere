...
However, this sort of $\V$-enriched Lawvere theory has arities for every finite object of $\V$.  These \textit{generalized arities} may be very powerful---rather than only inputting $n$-tuples of terms, we can input any finite object of terms.  Despite its potential, this generalization has not yet been exploited much in computer science.   Power \cite{powsketch} has introduced``enriched sketches'' as a way of presenting enriched Lawvere theories, and ``monads with arities'' have been explored by Berger, Melli\'es and Weber \cite{bergermelliesweber}, but they are still not yet widely understood or used.  What does it really mean for an operation to take in a finite graph of terms?  How can we learn to use this generality?  We hope that someone answers these questions, so that we can use more general arities in computer programming.
...
In this paper, however, we only consider \textit{natural number} arities, while still retaining enrichment. To do this we use the work of Lucyshyn-Wright \cite{lucyshyn-wright}, who has generalized
Power's ideas to allow a more flexible choice of arities.   For simplicity, we also limit ourselves 
to the case where the tensor product of $\V$ is cartesian.  

Thus, in all that follows, we let $(\V,\times,1_\V)$ be a cartesian closed category with finite coproducts of the terminal object $1_\V$.  Define $\NN_\V$ to be the full subcategory of $\V$ whose
objects are chosen finite coproducts of the terminal object:
\[   n_\V : = \sum_{i \in n} 1_\V  \]
These come with natural isomorphisms
\begin{equation}
\V(n_\V,a) \cong\V(1_\V,a)^n .
\end{equation}
We denote the inclusion by
\[ j \maps \NN_\V \hookrightarrow \V .\]
Associated to $\NN_\V$ there is a $\V$-category we call $\tilde{\J}$ whose objects are
those of $\NN_\V$ and whose hom-objects are given by
\[         \hom(a,b) = [j(a), j(b)]   .\]
...
\end{definition}
\noindent
This definition is adapted from that of Lucyshyn-Wright \cite{lucyshyn-wright}.  While he demands that $\tau$ be the identity on objects, we allow it to be bijective on objects, to handle change-of-base.
...
In the same way that all the objects of a Lawvere theory are $\NN$-powers of a single object, all the objects of a $\V$-theory are $\NN_\V$-powers $t^{n_\V}$ of the object $t = \tau(1_\V)$, one for each $n_\V \in \NN_\V$.  Note that $t$ itself is $t^{1_\V}$.  Moreover, just as every object of $\NN^\op$ is a power of $1$, every object of $\NN_\V^\op$ is a power of $1_\V$.  To see this, note that using Eq.\ (\ref{eq:co-power}), we obtain a natural isomorphism
\begin{equation}
n_\V \cong 1^{n_\V}
\end{equation}
in $\NN_\V^\op$.   While this may seem strange at first, it just the opposite of the isomorphism $n_\V \cong n_\V^1$ in $\NN_\V$.   Then, since $\tau$ preserves $\NN_\V$-powers and is bijective on objects, this implies that every object of $\T$ is an $\NN_\V$-power of $t = \tau(1_\V)$.

\begin{definition}
We define the \define{category of} $\V$\define{-theories}, $\V\Law$, to be the category for which an object is a $\V$-theory and a morphism from $(\T,\tau)$ to $(\T',\tau')$ is an $\NN_\V$-power-preserving $\V$-functors $f\maps \T \to \T'$ such that $f\tau = \tau'$. 
\end{definition}
...
If $\V$ is a \textbf{cosmos}, i.e.\ complete and cocomplete, then $\V\Law$ and $\Mod(\T,\C)$ naturally become $\V$-categories. This is potentially very useful, and the enriching categories $\V = \Gph, \Cat, \Pos, \Set$ that are the focus of this paper are indeed cosmoi.
...
\begin{example}
When $\V = \Cat$, a $\V$-theory deserves to be called a \define{2-theory}.  For example, let $\T = \Th(\mathrm{PsMon})$ be the 2-theory of pseudomonoids \cite{pseudo}.  A pseudomonoid is a weakened version of a monoid, so rather than associativity and unitality \textit{equations}, there are 2-isomorphisms called the \textbf{associator} and \textbf{unitors} which rewrite in the direction of a ``normal form''. To equate the multiple possible rewrite sequences, we need coherence laws Here is the presentation of the 2-theory:
	\[ \textbf{Th(PsMon)} \]
	\[\begin{array}{rll}
	...
	\textbf{coherence laws}
      ...
      \[\begin{tikzcd}
        ...
        \end{tikzcd}\]
    \end{example}
    %(M\times\alpha)\circ \alpha_{MmM}\circ (\alpha\times M) = \alpha_{MMm}\circ \alpha_{mMM} & \text{pentagon identity}\\ & (M\times \lambda) \circ \alpha_{M1M} = \rho\times M & \text{triangle identity}\\

Models of $\T$ in $\Cat$ are monoidal categories, and the induced 2-monad on $\Cat$ is the ``free monoidal category'' 2-monad. Let us explore this example in more detail.  A model of $\T$ is a finite-product-preserving 2-functor $\mu\colon \T\to \Cat$, which sends 
\[\begin{array}{rclcl}
	t & \mapsto & \C &&\\
