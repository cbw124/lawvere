...
where the left isomorphism arises from the Yoneda lemma, and the right isomorphism from the product preservation of $\mu$.   WHAT IS $\Mod$ HERE???  IS IT REALLY $\Mod(\T,\Set)$???  I think so.   If so, we should either write $\Mod(\T,\Set)$ or explain that we're using an abbreviation!!!
