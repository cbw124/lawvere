...
\begin{definition}
Given a system of arities $j \maps \J \hookrightarrow \V$, a $\V$-enriched algebraic theory with $j$-arities or \textbf{$\J$-$\V$ theory} $(\T,\tau)$ is a $\V$-category $\T$ equipped with an bijective-on-objects $\V$-functor 
\[ \tau\maps\tilde{\J}^\op \to \T \]
that is \textbf{$\J$-power preserving}: that is, it preserves powers by objects of (the subcategory image of) $\J$. 
\end{definition}
\noindent
