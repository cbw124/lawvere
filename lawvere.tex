...
In general for a cartesian closed category $\V$, a \textbf{$\V$-theory} is a $\V$-enriched Lawvere theory with natural number arities. We consider $\V$ as a choice of ``method of computation'' for algebraic theories. The main original idea and result is that product-preserving functors between enriching categories allow for the translation between different kinds of operational and denotational semantics -- this translation may be called ``change of computation'' or simply \textbf{change of base}.\\

\begin{remark}
Because operational semantics uses labelled directed (reflexive) graphs to represent terms and rewrites, one might expect $\Gph$, the category of directed graphs, to be our main example of enriching category; i.e. the ``thing'' of $n$-ary operations, or $n$-variable terms in a theory, should be a directed graph whose edges are rewrites. This is known as \textit{small step} operational semantics, meaning each edge represents a single instance of a rewrite rule.

When studying formal languages, one wants to pass from this local view to a global view -- given a term, one cares about its possible evolutions after not only one rewrite but a finite sequence of rewrites. We study how programs operate in finite time. In computer science, this corresponds to defining a rewrite relation and forming its transitive closure, called \textit{big step} operational semantics. This is the classic example which change of base aims to generalize.

However, there is a subtlety to this example. Categorically this translation corresponds to the ``free category'' functor $\FC\maps \Gph\to \Cat$, which for any graph forms a category whose objects are vertices and morphisms are finite sequences of edges. The fact is that this functor does \emph{not} preserve products -- essentially while $\FC$ turns reflexive loops into identities, it does not respect their unitality for a product of graphs, when these reflexive loops are components in a nonreflexive tuple. In other words, a product graph consists of parallel rewrites, and the free category functor does not equate ``rewrite both at once'' and ``rewrite one, then the other''.

Hence, the intuitive categorical interpretation of rewriting does not quite fit into the framework of syntax and semantics. To precisely represent operational semantics with enriched theories and models, we then need a more general concept known as a \emph{simplicial set}. While we do not need the greater generality in this paper, simplicial sets introduce the notion of higher-dimensional rewriting \cite{hdra}; this may prove to be a quite useful connection of subjects.

A simplicial set is a contravariant functor from the category of finite linear orders and monotone maps to the category of sets and functions. It can be visualized as a space built from ``simplices'', which generalize triangles to any dimension -- point, line, triangle, tetrahedron, etc. For an introduction to simplicial sets, see \cite{sset}. Denote by $\sSet := [\Delta^\op, \Set]$ the category of simplicial sets and natural transformations between them.

We consider simplicial sets which have only ``reflexivities'' above dimension 1, and finite products thereof. For our purposes, these can be thought of as ordinary directed multigraphs. If the reader does not know or learn about simplicial sets, one only needs to know that ours are like graphs equipped with a functor $R\maps \sSet\to \Cat$, which is similar to $\FC$ except that it preserves products.
\end{remark}
...
There is a spectrum of choices for $\V$, which allow us to examine the computation of term calculi at various levels of detail.\\
\[\begin{array}{lll}
...
\textbf{Categories} & \Cat\text{-theories represent ``big-step'' operational semantics:} \\  & \text{(Often this means a rewrite to a \textit{normal form}. We use the term more generally.)}\\ & \text{--- a morphism is a \textit{finite sequence} of rewrites.}\\
...
% \begin{itemize}
% \item 
% \textbf{Simplicial Sets}: $\sSet$-theories represent ``small-step" operational semantics: \\ --- an edge is a \textit{single} term rewrite.
% \item
% \textbf{Categories}: $\Cat$-theories represent ``big-step" operational semantics. \\  (Often this means a rewrite to a \textit{normal form}. We use the term more generally.)\\ --- a morphism is a \textit{finite sequence} of rewrites. 
% \item
% \textbf{Posets}: $\Pos$-theories represent ``full-step" operational semantics:\\ --- a boolean is the \textit{existence} of a big-step rewrite.
% \item
% \textbf{Sets}: $\Set$-theories represent denotational semantics: \\ --- an element is an \textit{connected component} of the rewrite relation.
% \end{itemize}
...
\sSet \arrow[bend right,swap]{rr}{R}
& \ld &
...
$R \maps \sSet \to \Cat$, called \textit{realization}, maps a simplicial set $G$ to the category whose objects are vertices of $G$, whose morphisms are sequences of composable edges in $G$, and composition is concatenation, modulo equations for associativity and unitality. Change of base along $R$ maps small-step operational semantics to big-step operational semantics.
...
$\FS \maps \Pos \to \Set$ maps a poset $P$ to the set of ``components'' of $P$, where $p,p' \in P$ are in the same component if $p \le p'$. Change of base along $\FS$ maps full-step operational semantics to denotational semantics.
...
Hence, any cartesian functor that preserves chosen finite coproducts of the terminal object gives a \textbf{change of base}.  That is, it provides for a method of translating formal languages between various ``modes of operation''.   Moreover, this reasoning generalizes to \textit{multisorted} $\V$-theories, enriched theories which have multiple sorts: given any $n\in \mathbb{N}$, the monoidal subcategory $(\NN_\V)^n$ is also an eleutheric system of arities, so Lucyshyn-Wright's monadicity theorem still applies.   In Section \ref{ssec:bisimulation} we show how this is useful in the study of bisimulation.
...
\bibitem{hdra} Y. Guiraud and P. Malbos, Higher Dimensional Rewriting and Algebra. Workshop at \href{http://hdra.gforge.inria.fr/}.
